...
function material(T: any, color: number, emissive = 0, roughness = 0.55, metalness = 0.15) { return new T.MeshStandardMaterial({ color, emissive, emissiveIntensity: emissive ? 1.3 : 0, roughness, metalness }); }
function add(T: any, parent: any, geometry: any, mat: any, pos: [number, number, number], scale: [number, number, number] = [1, 1, 1]) { const m = new T.Mesh(geometry, mat); m.position.set(...pos); m.scale.set(...scale); m.castShadow = true; m.receiveShadow = true; parent.add(m); return m; }
function createCharacter(T: any) {
  const root = new T.Group(); const g: Record<string, any> = {};
  const white = material(T, 0xeaf6ff, 0x193c66, 0.35, 0.3), dark = material(T, 0x0a1120, 0x021a36, 0.22, 0.7), cyan = material(T, 0x55e9ff, 0x00d9ff, 0.2, 0.55), skin = material(T, 0xf4d4d2, 0x18090c, 0.62, 0.05), eye = material(T, 0x8eefff, 0x00ddff, 0.12, 0.2), black = material(T, 0x02040a);
  ...
  const halo = add(T, g.head, new T.TorusGeometry(0.48, 0.027, 8, 48), cyan, [0, 0.55, 0], [1, 0.52, 0.62]); halo.rotation.x = Math.PI / 2;
  ...
  g.hip = new T.Group(); g.hip.position.y = -0.7; g.body.add(g.hip); add(T, g.hip, new T.ConeGeometry(0.72, 0.45, 9, 1, true), white, [0, 0.02, 0], [1, 1, 0.72]); add(T, g.hip, new T.TorusGeometry(0.55, 0.045, 8, 28), cyan, [0, -0.17, 0], [1, 1, 0.72]);
  g.armL = new T.Group(); g.armR = new T.Group(); g.armL.position.set(-0.72, 0.3, 0); g.armR.position.set(0.72, 0.3, 0); g.body.add(g.armL, g.armR); add(T, g.armL, new T.CylinderGeometry(0.13, 0.17, 0.62, 14), white, [0, -0.3, 0]); add(T, g.armR, new T.CylinderGeometry(0.13, 0.17, 0.62, 14), white, [0, -0.3, 0]);
  g.foreL = new T.Group(); g.foreR = new T.Group(); g.foreL.position.y = -0.61; g.foreR.position.y = -0.61; g.armL.add(g.foreL); g.armR.add(g.foreR); add(T, g.foreL, new T.CylinderGeometry(0.115, 0.145, 0.6, 14), dark, [0, -0.3, 0]); add(T, g.foreR, new T.CylinderGeometry(0.115, 0.145, 0.6, 14), dark, [0, -0.3, 0]); add(T, g.foreL, new T.SphereGeometry(0.14, 14, 10), white, [0, -0.61, 0]); add(T, g.foreR, new T.SphereGeometry(0.14, 14, 10), white, [0, -0.61, 0]);
  g.thighL = new T.Group(); g.thighR = new T.Group(); g.thighL.position.set(-0.3, -0.98, 0); g.thighR.position.set(0.3, -0.98, 0); g.hip.add(g.thighL, g.thighR); add(T, g.thighL, new T.CylinderGeometry(0.16, 0.13, 0.68, 14), white, [0, -0.33, 0]); add(T, g.thighR, new T.CylinderGeometry(0.16, 0.13, 0.68, 14), white, [0, -0.33, 0]);
  g.calfL = new T.Group(); g.calfR = new T.Group(); g.calfL.position.y = -0.68; g.calfR.position.y = -0.68; g.thighL.add(g.calfL); g.thighR.add(g.calfR); add(T, g.calfL, new T.CylinderGeometry(0.12, 0.1, 0.7, 14), dark, [0, -0.35, 0]); add(T, g.calfR, new T.CylinderGeometry(0.12, 0.1, 0.7, 14), dark, [0, -0.35, 0]);
...
function resetRig(c: any) { const g = c.g; g.body.position.y = 1.7; g.body.rotation.set(0, 0, 0); g.head.rotation.set(0, 0, 0); g.hip.rotation.set(0, 0, 0); g.armL.rotation.set(0, 0, 0.08); g.armR.rotation.set(0, 0, -0.08); g.foreL.rotation.set(0, 0, 0); g.foreR.rotation.set(0, 0, 0); g.thighL.rotation.set(0, 0, 0); g.thighR.rotation.set(0, 0, 0); g.calfL.rotation.set(0, 0, 0); g.calfR.rotation.set(0, 0, 0); }
function animateRig(c: any, action: Action, t: number, level: number, mood: AvatarMood) {
  const g = c.g; resetRig(c); const walk = Math.sin(t * 8), alt = Math.sin(t * 8 + Math.PI); g.body.position.y += Math.sin(t * 2.3) * 0.025;
  ...
  g.mouth.scale.y = level > 0.02 ? 0.45 + Math.min(level, 1) * 0.65 : 0.45; c.cyan.emissiveIntensity = level > 0.02 ? 1.3 + Math.min(level, 1) * 3.2 : 1.25; const sleeping = mood === "sleep" && action === "sit"; c.eyes.forEach((e: any) => { e.scale.y = sleeping ? 0.12 : 1; });
}
export function JarvisCompanion({ state, mood, level }: Props) {
  ...
  useEffect(() => {
    const mount = mountRef.current, T = window.THREE; if (!mount || !T) return;
