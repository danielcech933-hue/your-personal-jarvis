...
function expression(vrm: any, name: string, value: number) {
  try {
    ...
    // Model-dependent expression names are optional.
  }
}
...
function animate(vrm: any, pose: Record<string, any>, action: Action, t: number, level: number, mood: AvatarMood) {
  ...
  if (action === "walk") {
    ...
    rotate(pose, "spine", Math.sin(t * 3.4) * 0.02);
  } else if (action === "wave") {
    ...
    rotate(pose, "chest", -0.09, 0, Math.sin(t * 1.2) * 0.04);
    ...
    rotate(pose, "leftUpperArm", Math.sin(t * 0.8) * 0.02, 0, 0.025);
    rotate(pose, "rightUpperArm", Math.sin(t * 0.8 + Math.PI) * 0.02, 0, -0.025);
  }

  if (action === "spin") rotate(pose, "chest", 0, t * 3.6, 0);
...
function visibleFallback(action: string) {
  return (
    <div className="jarvis-avatar-fallback" aria-label="Živá 3D postava se načítá">
      ...
      <div className="fallback-character" data-action={action}>
        ...
        <div className="fallback-head"><span className="fallback-eye left" /><span className="fallback-eye right" /><span className="fallback-mouth" /></div>
        <div className="fallback-neck" />
        <div className="fallback-torso"><span className="fallback-core" /></div>
        <div className="fallback-arm left" /><div className="fallback-arm right" />
        <div className="fallback-skirt" />
        <div className="fallback-leg left" /><div className="fallback-leg right" />
      </div>
    </div>
  );
}
...
export function JarvisCompanion({ state, mood, level, onActivity }: Props) {
  ...
  const [fallbackAction, setFallbackAction] = useState("idle");
  ...
  useEffect(() => {
    ...
    const fallbackOnly = () => {
      ...
      let until = started + 2600;
      const choose = (now: number) => {
        const p = propsRef.current;
        if (p.state !== "idle") {
          action = "think";
          until = now + 700;
        } else if (p.mood === "sleep") action = "sleep";
        else if (p.mood === "play") action = (["dance", "jump", "wave", "spin"] as Action[])[Math.floor(Math.random() * 4)] || "dance";
        else if (p.mood === "bored") action = (["sit", "stretch", "think", "walk"] as Action[])[Math.floor(Math.random() * 4)] || "sit";
        else action = (["idle", "walk", "work", "stretch", "wave"] as Action[])[Math.floor(Math.random() * 5)] || "idle";
        started = now;
        until = now + (action === "work" ? 6000 : action === "sleep" ? 7500 : 1800 + Math.random() * 2800);
        setAction(action);
      };
      choose(started);
    ...
    const boot = async () => {
      ...
      try {
        ...
        scene.add(new T.HemisphereLight(0xc9efff, 0x07111b, 2));
        const key = new T.DirectionalLight(0xffffff, 3.4); key.position.set(-3, 6, 5); key.castShadow = true; scene.add(key);
        const rim = new T.PointLight(0x22ddff, 13, 12); rim.position.set(-3, 2.8, 2); scene.add(rim);
        const warm = new T.PointLight(0xffb66a, 3.5, 9); warm.position.set(3, 1.8, 2); scene.add(warm);
        ...
        vrm.scene.traverse((o: any) => { o.castShadow = true; o.receiveShadow = true; });
        ...
        let x = 0, z = 0, tx = 0, tz = 0, yaw = 0;
        let action: Action = "idle";
        let started = performance.now() / 1000;
        let until = started + 2.5;
        let mouseX = 0, mouseY = 0;

        const choose = (now: number) => {
          const p = propsRef.current;
          if (p.state !== "idle") action = p.state === "thinking" ? "think" : "idle";
          else if (p.mood === "sleep") action = "sleep";
          else if (p.mood === "play") action = (["dance", "walk", "jump", "spin", "flip", "wave", "stretch"] as Action[])[Math.floor(Math.random() * 7)] || "dance";
          else if (p.mood === "bored") action = (["walk", "sit", "think", "stretch", "wave"] as Action[])[Math.floor(Math.random() * 5)] || "sit";
          else if (p.mood === "curious") action = (["walk", "think", "wave", "stretch", "spin"] as Action[])[Math.floor(Math.random() * 5)] || "think";
          else action = (["walk", "work", "sit", "stretch", "think", "wave", "idle"] as Action[])[Math.floor(Math.random() * 7)] || "idle";
          started = now;
          until = now + (action === "walk" ? 4.6 + Math.random() * 2.8 : action === "work" ? 6 + Math.random() * 4 : action === "sleep" ? 7 + Math.random() * 7 : 1.7 + Math.random() * 2.6);
          if (["walk", "work", "sit"].includes(action)) {
            tx = action === "work" ? -1.8 + (Math.random() - 0.5) * 0.25 : -2.15 + Math.random() * 3.8;
            tz = action === "sit" ? 0.25 : -0.7 + Math.random() * 1.35;
          }
          if (["jump", "flip"].includes(action)) {
            tx = clamp(x + (Math.random() - 0.5), -2.25, 2.25);
            tz = clamp(z + (Math.random() - 0.5) * 0.6, -0.75, 0.75);
          }
          setAction(action);
        };
        ...
        const observer = new ResizeObserver(onResize); observer.observe(mount);
        choose(performance.now() / 1000);
        let previous = performance.now();
        const frame = (ms: number) => {
          if (cancelled) return;
          const now = ms / 1000;
          const dt = Math.min((ms - previous) / 1000, 0.05); previous = ms;
          const p = propsRef.current;
          if (now >= until) choose(now);
          if (action === "walk" || action === "work" || action === "sit") {
            const dx = tx - x, dz = tz - z, d = Math.hypot(dx, dz);
            if (d > 0.03) { const speed = action === "work" ? 0.7 : 0.95; const step = Math.min(speed * dt, d); x += dx / d * step; z += dz / d * step; yaw = Math.atan2(dx, dz); }
          }
          x = clamp(x, -2.45, 2.45); z = clamp(z, -0.92, 0.92);
          vrm.scene.position.x = x; vrm.scene.position.z = z;
          if (action === "jump") vrm.scene.position.y = Math.sin(clamp((now - started) / 1.15, 0, 1) * Math.PI) * 0.45;
          else if (action === "flip") { const phase = clamp((now - started) / 1.7, 0, 1); vrm.scene.position.y = Math.sin(phase * Math.PI) * 0.34; vrm.scene.rotation.x = phase * Math.PI * 2; }
          else { vrm.scene.position.y = 0; vrm.scene.rotation.x = 0; }
          if (action === "spin") vrm.scene.rotation.y = yaw + clamp((now - started) / Math.max(until - started, 0.01), 0, 1) * Math.PI * 2;
          else vrm.scene.rotation.y += clamp(yaw - vrm.scene.rotation.y, -0.09, 0.09);
          animate(vrm, pose, action, now - started, p.level, p.mood);
          if (pose.head?.bone) { pose.head.bone.rotation.y = clamp(mouseX * 0.18, -0.18, 0.18); pose.head.bone.rotation.x += clamp(-mouseY * 0.06, -0.06, 0.06); }
  ...
  const stateAction = state === "sleep" ? "sleep" : state === "thinking" ? "think" : fallbackAction;
  return <div ref={mountRef} className="jarvis-3d-layer">{visibleFallback(stateAction)}</div>;
}
