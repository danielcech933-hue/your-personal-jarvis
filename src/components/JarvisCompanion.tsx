...
function createCharacter(T: any) {
  ...
  const white = material(T, 0xeaf6ff, 0x193c66, 0.35, 0.3);
  const dark = material(T, 0x0a1120, 0x021a36, 0.22, 0.7);
  const cyan = material(T, 0x55e9ff, 0x00d9ff, 0.2, 0.55);
  const gold = material(T, 0xd6b36c, 0x3b2400, 0.3, 0.72);
  const skin = material(T, 0xf4d4d2, 0x18090c, 0.62, 0.05);
  const eye = material(T, 0x8eefff, 0x00ddff, 0.12, 0.2);
  const black = material(T, 0x02040a);
  ...
  g.eyeL = add(T, g.head, new T.SphereGeometry(0.095, 16, 12), eye, [-0.2, 0.02, -0.56], [0.85, 1.4, 0.45]);
  g.eyeR = add(T, g.head, new T.SphereGeometry(0.095, 16, 12), eye, [0.2, 0.02, -0.56], [0.85, 1.4, 0.45]);
  add(T, g.head, new T.SphereGeometry(0.028, 10, 8), black, [0, -0.15, -0.58]);
  g.mouth = add(T, g.head, new T.SphereGeometry(0.037, 12, 8), black, [0, -0.14, -0.58], [1.9, 0.45, 0.4]);
  add(T, g.head, new T.TorusGeometry(0.48, 0.027, 8, 48), cyan, [0, 0.55, 0], [1, 0.52, 0.62]).rotation.x = Math.PI / 2;
  add(T, g.head, new T.TorusGeometry(0.17, 0.025, 8, 28), cyan, [-0.6, 0.03, 0], [1, 1.15, 0.72]);
  add(T, g.head, new T.TorusGeometry(0.17, 0.025, 8, 28), cyan, [0.6, 0.03, 0], [1, 1.15, 0.72]);

  g.hip = new T.Group(); g.hip.position.y = -0.7; g.body.add(g.hip);
  add(T, g.hip, new T.ConeGeometry(0.72, 0.45, 9, 1, true), white, [0, 0.02, 0], [1, 1, 0.72]);
  add(T, g.hip, new T.TorusGeometry(0.55, 0.045, 8, 28), cyan, [0, -0.17, 0], [1, 1, 0.72]);
  ...
  add(T, g.foreL, new T.CylinderGeometry(0.115, 0.145, 0.6, 14), dark, [0, -0.3, 0]); add(T, g.foreR, new T.CylinderGeometry(0.115, 0.145, 0.6, 14), dark, [0, -0.3, 0]);
  add(T, g.foreL, new T.SphereGeometry(0.14, 14, 10), white, [0, -0.61, 0]); add(T, g.foreR, new T.SphereGeometry(0.14, 14, 10), white, [0, -0.61, 0]);
  ...
  g.bootL = add(T, g.calfL, new T.CapsuleGeometry(0.18, 0.28, 6, 12), white, [0, -0.78, -0.17], [1, 0.55, 1.45]);
  g.bootR = add(T, g.calfR, new T.CapsuleGeometry(0.18, 0.28, 6, 12), white, [0, -0.78, -0.17], [1, 0.55, 1.45]);
...
function resetRig(c: any) {
  const g = c.g;
  g.body.position.y = 1.7; g.body.rotation.set(0, 0, 0); g.head.rotation.set(0, 0, 0); g.hip.rotation.set(0, 0, 0);
  g.armL.rotation.set(0, 0, 0.08); g.armR.rotation.set(0, 0, -0.08); g.foreL.rotation.set(0, 0, 0); g.foreR.rotation.set(0, 0, 0);
  g.thighL.rotation.set(0, 0, 0); g.thighR.rotation.set(0, 0, 0); g.calfL.rotation.set(0, 0, 0); g.calfR.rotation.set(0, 0, 0); g.bootL.rotation.set(0, 0, 0); g.bootR.rotation.set(0, 0, 0);
}

function animateRig(c: any, action: Action, t: number, level: number, mood: AvatarMood) {
  const g = c.g; resetRig(c); const walk = Math.sin(t * 8); const alt = Math.sin(t * 8 + Math.PI);
...
export function JarvisCompanion({ state, mood, level }: Props) {
  ...
  useEffect(() => {
    ...
    const camera = new T.PerspectiveCamera(32, innerWidth / innerHeight, 0.1, 100); camera.position.set(0, 2.7, 8.7); camera.lookAt(0, 1.8, 0);
