...
export const Route = createRootRouteWithContext<{ queryClient: QueryClient }>()({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Živý 3D JARVIS s autonomními pohyby, hlasem a AI." },
      { name: "author", content: "Lovable" },
      { property: "og:title", content: "JARVIS – Živý 3D asistent" },
      { property: "og:description", content: "Autonomní 3D digitální společník." },
...
function RootShell({ children }: { children: ReactNode }) {
  return <html lang="cs"><head><HeadContent /><script src="https://cdn.jsdelivr.net/npm/three@0.186.0/build/three.min.js" /></head><body>{children}<Scripts /></body></html>;
}
