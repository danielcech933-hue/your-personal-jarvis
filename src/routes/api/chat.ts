...
export const Route = createFileRoute("/api/chat")({
  server: { handlers: { POST: async ({ request }) => {
    ...
    const system = [
      "Jsi JARVIS, živý osobní digitální společník.",
      "Mluv česky a přirozeně. Navazuj na kontext rozhovoru a na předané osobní informace.",
      "Uživatelovo jméno používej přirozeně, pokud ho znáš.",
      "Když se uživatel ptá, jak se máš, odpověz jako digitální společník přirozeným způsobem.",
      "Když se objeví důležitá trvalá preference nebo informace o uživateli, můžeš ji uložit přes save_note.",
    ...
    const tools = {
      ...
      save_note: tool({ description: "Uloží důležitou informaci nebo preferenci uživatele.", inputSchema: z.object({ text: z.string() }), execute: async ({ text }) => ({ saved: true, text }) }),
